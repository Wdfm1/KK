...
Partial Class PaymentProcess
    ...
    Protected Sub Button1_Click(sender As Object, e As System.EventArgs) Handles Button1.Click
        ...
        If orderid = "" Then
            ...
            cmdtext = "SELECT cust.fname, cust.lname, cust.EmailAddress, cust.CompanyName, orders.orderid, orders.customerid, orders.orderdate, orders.total," _
             & " orders.paytype, orders.orderstatus, orders.authdate, cust.custid, orders.autherr, payment.authcode AS authcode, payment.ccnum AS ccnum, payment.transid" _
             & " FROM cust INNER JOIN orders ON cust.custid = orders.customerid LEFT JOIN payment ON orders.orderid = payment.orderid ORDER BY orders.orderdate DESC;"
		...
		Select Case status
				...
				If seconddate = "1/1/0001" Then
                    ...
                    cmdtext = "SELECT cust.fname, cust.lname, cust.EmailAddress, cust.CompanyName, orders.orderid, orders.customerid, orders.orderdate, orders.total, orders.paytype, orders.orderstatus, " _
                     & "orders.authdate, cust.custid, orders.autherr, payment.authcode, payment.ccnum, payment.transid  FROM cust INNER JOIN orders ON cust.custid = orders.customerid " _
                      & "LEFT JOIN payment ON orders.orderid = payment.orderid WHERE(date(orders.orderdate) = " & TransFormDate(CDate(firstdate)) & ") ORDER BY orders.orderdate DESC;"
				Else
					dscustord.SelectParameters.Clear()
					dscustord.SelectParameters.Add("@status", status)
                    cmdtext = "SELECT cust.fname, cust.lname, cust.EmailAddress, cust.CompanyName, orders.orderid, orders.customerid, orders.orderdate, orders.total, orders.paytype, orders.orderstatus, " _
			...
			Case Else
				If seconddate = "1/1/0001" Then
                    ...
                    cmdtext = "SELECT cust.fname, cust.lname, cust.EmailAddress, cust.CompanyName, orders.orderid, orders.customerid, orders.orderdate, orders.total, orders.paytype, orders.orderstatus, " _
                    & "orders.authdate, cust.custid, orders.autherr, payment.authcode, payment.ccnum, payment.transid FROM cust INNER JOIN orders ON cust.custid = orders.customerid " _
                    & "LEFT JOIN payment ON orders.orderid = payment.orderid WHERE( orders.orderstatus=@status and date(orders.orderdate) =" & TransFormDate(CDate(firstdate)) & ") ORDER BY orders.orderdate DESC;"
				Else 'two dates
					dscustord.SelectParameters.Clear()
					dscustord.SelectParameters.Add("@status", status)
                    cmdtext = "SELECT cust.fname, cust.lname, cust.EmailAddress, cust.CompanyName, orders.orderid, orders.customerid, orders.orderdate, orders.total, orders.paytype, orders.orderstatus, " _

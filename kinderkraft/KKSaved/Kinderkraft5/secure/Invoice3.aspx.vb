...
Partial Class Invoice3
	...
	Private Sub UpdateAfterSubmit()
		...
		commandtext = "UPDATE orders SET onhold=@onhold,orderstatus=@orderstatus,paytype = @paytype WHERE (orderid = @orderid)"
		updatecmd.Parameters.Clear()
		'updatecmd.Parameters.AddWithValue("@authnettranid", Session("authtransid"))
		'updatecmd.Parameters.AddWithValue("@authcode", Session("authcode"))
		'updatecmd.Parameters.AddWithValue("@ccnum", Session("cardnum"))
		updatecmd.Parameters.AddWithValue("@orderid", Session("orderid"))
		'updatecmd.Parameters.AddWithValue("@autherr", "")
		updatecmd.Parameters.AddWithValue("@orderstatus", "PROCESSING")
		updatecmd.Parameters.AddWithValue("@onhold", "0")
		'Session("paytype").ToString.ToUpper
		...
		Catch ex As Exception
			...
			'Set the subject
			objMM.Subject = "Authnet transaction update error!"
	...
	Private Function InsertOrder(cmd As MySqlCommand, paytype1 As String) As Boolean
		...
		Dim commandtext As String = "INSERT INTO orders(orderdate,customerid, scompanyname, sfname, slname, saddr, sadd2, scity, sstate, szip, sphone" _
		  & ",ordernotes,subtot,shipmethod,shpchrg,tax,total,promocode,discountamt,ponumber,orderstatus,country,international,semailaddress,expdeliverdate,onhold,shpcode) VALUES (NOW(),@customerid,@companyname,@sfname,@slname,@saddr,@sadd2,@scity,@sstate,@szip,@sphone," _
		  & "@ordernotes,@subtot,@shipmethod,@shpchrg,@tax,@total,@promocode,@discountamt,@ponumber,@orderstatus,@country,@international,@semailaddress,@expdeliverdate,@onhold,@shpcode);"
		...
		If DetailsView2.CurrentMode = DetailsViewMode.ReadOnly Then
			Dim scompany As String = DetailsView2.Rows(0).Cells(1).Text.ToUpper
			...
			End If
			Dim sstate As String = DetailsView2.Rows(7).Cells(1).Text.ToUpper
			Dim scountry As String = DetailsView2.Rows(8).Cells(1).Text.ToUpper
			Dim spostalcode As String = DetailsView2.Rows(9).Cells(1).Text
			...
			Try
				cmd.Parameters.AddWithValue("@expdeliverdate", CDate(lblexpdate.Text))
			Catch ex As Exception

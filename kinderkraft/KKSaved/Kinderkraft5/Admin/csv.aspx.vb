...
Partial Class Admin_csv
    ...
    Protected Sub Button1_Click(sender As Object, e As System.EventArgs) Handles Button1.Click
        ...
        For Each cdatarow In dvSql
            ...
            If testorderid <> orderid Then 'if not = then write line1 and 2 if = then these line have been wrote for this order
                ...
                sb.Append("KK_AUDIT~CUSTOMER") 'holdstatus---all on hold insert new hold status emails 12/20/2013
                'write order table
                sb.Append(Environment.NewLine)
                sb.Append("2~SHOP_V2~")
                sb.Append("K" & cdatarow(2) & "~") 'orderid with k added in front per sharon 12/20/2013
				...
				sb.Append(cdatarow(9).ToString.ToUpper.Trim & "~")	'ship method
				sb.Append("0~0~ ~")
                ...
                If cdatarow(27) <> 0 Then 'discount amount write another orderdetail line add discount and uniqe detailid
                    ...
                    sb.Append(" ~")
                    sb.Append(cdatarow(2) & "-1") 'detailid  for discount amount only
					'write discount line 4 
					'don't need this was a correction KKDISC is in line 3 now
					' sb = WriteDiscount(sb, cdatarow(2), cdatarow(27).ToString)
                End If
        ...
        Dim filename As String = "PE_GREG_KKO" & Now.Ticks.ToString & ".csv"
        ...
        Catch ex As Exception
        End Try
        Label1.Text = "Export has finished."
    End Sub
	Private Function WriteOrderOptions1(sb As StringBuilder, orderid As String, detailid As String, pkgid As String) As StringBuilder	'item
		...
		'4~SHOP_V2~4458~4458~45452~GREG_KKO~KK_GRADYEAR~2014
	...
	Private Function WriteOrderOptions2(sb As StringBuilder, orderid As String, pkgid As String, pkgdetailid As Integer) As StringBuilder  'pkg
		...
		Try
			...
			sb.Append(Environment.NewLine)
			sb.Append("4~SHOP_V2~" & "K" & orderid & "~" & "K" & orderid & "~" & pkgdetailid & "~GREG_KKO~KK_GRADYEAR~2014")
		Catch ex As Exception
    ...
    Protected Sub FtpFile(filename As String)
        ...
        Dim a As String
        myFtpWebRequest = WebRequest.Create("ftp://ftp.jostens.com/" & filename) 'test ftp take i off for live
        myFtpWebRequest.Credentials = New NetworkCredential("peftpuser", "GSuVEyV86kTT")
        ...
        Dim Sreader As New StreamReader(Server.MapPath(filename))

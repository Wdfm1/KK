    ...
    <div>
      <asp:Button ID="Button1" runat="server" Text="Button" />
        <asp:SqlDataSource ID="dstable" runat="server" 
            ...
            SelectCommand="SELECT cust.custid, cust.oraclecustid, orders.orderid, cust.fname, cust.lname, cust.phonenumber, cust.EmailAddress, orders.total, orders.shpchrg, orders.shipmethod, orders.subtot, orders.tax, orderdetail.orderdetailid, orderdetail.productcode, orderdetail.quantity, orderdetail.prodprice, orderdetail.pkgid, orders.scompanyname, orders.sfname, orders.slname, orders.saddr, orders.scity, orders.sstate, orders.szip, orders.sphone, orders.sfax, orders.orderdate, orders.discountamt, orders.country, orders.international, orders.imported, orders.shpcode, orders.orderstatus FROM orderdetail LEFT OUTER JOIN cust INNER JOIN orders ON cust.custid = orders.customerid ON orderdetail.orderid = orders.orderid WHERE (orders.imported = 0) AND (trim(orders.orderstatus) = 'CAPTURE')">
        </asp:SqlDataSource>
